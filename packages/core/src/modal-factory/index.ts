import { createNoopStoreAdapter } from "../noop-store"
...
import { isNotEmpty, merge } from "../shared/utils"
import type {
  AbstractFnToMiddleware,
  AnotherModalCreator,
  AnyModalCreator,
    ...
    .reduce((prev, [key, middlewares]) => {
      prev[key] = <Type extends string>(type: Type) => {
        ...
        return middlewares.reduce(
          (context, middleware) => {
            const appliedOneRule = context.builder.use(middleware) as typeof createdModal

            Object.assign(appliedOneRule, {
              withParams: () => appliedOneRule
            })
...
export const combine = <M extends AnyArrowFn>(callback: M) => {
  return <Params extends Modal.params>(params: Params) => {
    return callback(params) as AbstractFnToMiddleware<M> extends Modal.middleware<any, infer Context, infer Payload>
...
export const toBrand = {
  payload: <Payload extends AnyPayloadBrand>(params: PayloadUnbrand<Payload>) => params as Payload,
  store: <Store extends AnyStoreBrand>(params: StoreUnbrand<Store>) => params as Store,
}
...
export namespace Modal {
  ...
  export type params = GetParameters<Middleware<AnotherModalCreator, any, any>>
}
